...
opensourceai_IMAGE_TAG=`cat ${CLUSTER_CONFIG} | grep docker_image_tag | tr -d "[:space:]" | cut -d ':' -f 2`

echo "opensourceai Image Tag ${opensourceai_IMAGE_TAG}"
...
sudo docker run -itd \
        ...
        opensourceai/dev-box:${opensourceai_IMAGE_TAG}

echo "Checking k8s installation..."
sudo docker exec dev-box-quick-start kubectl get node

echo "Starting opensourceai service with dev-box..."
...
echo ""
echo "opensourceai is successfully deployed, please check the following information:"
echo "Kubernetes cluster config :     ~/pai-deploy/kube/config"
echo "opensourceai cluster config    :     ~/pai-deploy/cluster-cfg"
echo "opensourceai cluster ID        :     pai"
