...
def main():
    ...
    if 'opensourceai_kube_network_plugin' not in cluster_config or cluster_config['opensourceai_kube_network_plugin'] != 'weave':
        count_input = 0
        while True:
            ...
            if count_input == 3:
                ...
                sys.exit(1)
        if user_input == "Y" \
            and ('opensourceai_kube_network_plugin' not in cluster_config or cluster_config['opensourceai_kube_network_plugin'] == 'calico'):
            logger.error("Azure does not support calico, please change the opensourceai_kube_network_plugin to weave")
    ...
    generate_template_file(
        "quick-start/opensourceai.yml.template",
        "{0}/opensourceai.yml".format(output_path),
        map_table
    )
