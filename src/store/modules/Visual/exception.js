...
const actions = {
  ...
  async fetchAllStep(context) {
    ...
    if (
      context.state.curRunTag.step === '' ||
      allStepTemp[0][2].step.indexOf(context.state.curRunTag.step) === -1
    ) {
  ...
  async fetchData(context) {
    ...
    await http.useGet(port.category.exception_data, param).then((res) => {
      ...
      if (!(rectDataTemp[4][0] instanceof Array)) {
        // 处理一维数据
  ...
  async fetchAllData(context) {
    // 获得初始数据,step均为step[0], param={run, tag, step}，run、tag发生变化
  ...
  async fetchOneData(context, param) {
    // {run: param.run, tag: param.tag, step: param.step, index}，step发生变化
...
const mutations = {
  ...
  setIntervalSelfCategoryInfo(state, param) {
    ...
    if (
      state.curRunTag === null ||
      state.run.indexOf(state.curRunTag.run) === -1 ||
      state.allStep.length === 0
    ) {
