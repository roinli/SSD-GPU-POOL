...
const actions = {
  ...
  async fetchAttentionMap(context, param) {
    if (param !== undefined) {
      const res = await http.useGet(port.category.transformer_image, param);
...
const mutations = {
  ...
  setTransformerTextData: (state, param) => {
    ...
    const layersLength = param[1][param[0]].data.attention.all.attn.length;
  ...
  setAttnData(state, param) {
    if (Object.keys(param).includes('num_layers')) {
      ...
      state.attnMap[param[Object.keys(param)].layer] = param[Object.keys(param)].attn_map;
