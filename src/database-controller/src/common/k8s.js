...
async function getFramework(name, namespace = 'default') {
  const res = await customObjectsClient.getNamespacedCustomObject(
    'frameworkcontroller.opensource-china.com',
    'v1',
    namespace,
    'frameworks',
    name,
...
function getFrameworkInformer(
  ...
  */
  ...
  const informer = k8s.makeInformer(
    kc,
    `/apis/frameworkcontroller.opensource-china.com/v1/namespaces/${namespace}/frameworks?timeoutSeconds=${timeoutSeconds}`,
    listFn,
  );
  return informer;
}
...
async function patchSecretOwnerToFramework(secret, frameworkResponse) {
  const metadata = {
    ownerReferences: [
      {
        apiVersion: 'frameworkcontroller.opensource-china.com',
        kind: 'Framework',
