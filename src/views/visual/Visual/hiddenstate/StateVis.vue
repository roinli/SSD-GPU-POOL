  ...
  <div class="state-vis">
    <div class="header">
      ...
      <div class="move-button-zrea">
        <div class="move-left">
          <button @click="turnToleft">
            <img src="@/assets/VisualImg/arrow-left-circle.svg" style="height: 20px;" />
          </button>
        </div>
        <div class="move-right">
          <button @click="turnToRight">
            <img src="@/assets/VisualImg/arrow-right-circle.svg" style="height: 20px;" />
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="getStateMatchData.length" class="match-content">
      <el-table
        class="match-content-table"
        :data="getStateMatchData"
        stripe
        border
        style="width: 100%;"
      ...
      >
        ...
        <el-table-column prop="data" label="Message" align="center"> </el-table-column>
        <el-table-column prop="start_pos" label="Start pos" width="100px" align="center">
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...mapHiddenStateActions(['getHiddenStateData']),
    ...
    drawStateVisBox() {
      ...
      // 隐状态显示区域
      const stateVisBox = d3
        .select('.state-vis-box')
        .append('svg')
      ...
      if (Array.isArray(this.getSelectedLineIndexs) && this.getSelectedLineIndexs.length) {
        const highlightedValueLinesData = this.jointData.filter((d) =>
          _.includes(this.getSelectedLineIndexs, d.dim)
        );
        const lineOpacity = this.lineOpacity(highlightedValueLinesData);
        const lineGenerator = this.lineGenerator(xScale, yScale);
        ...
        overlay
          .selectAll('.valueLine')
          .on('mouseenter', (d) => {
            overlay.selectAll('.valueLine').classed('hovered', (hoveredLine) => {
              return hoveredLine.dim === d.dim;
            });
          })
          .on('mouseout', (d) => {
            overlay.selectAll('.valueLine').classed('hovered', false);
          })
          .on('mouseover', (d) => {
          ...
          .on('mousemove', (d) => {
            if (!this.isChangingThreshold) {
              Tooltip.style('left', `${d3.event.pageX - 120}px`).style(
                'top',
    ...
    watchValueChange() {
      ...
      if (this.getSelectedRange.length && this.bottomBrushSelection.length) {
        ...
        const signature = this.selectedRangeIndexs(leftBound, rightBound)
        ...
        this.jointData.forEach((lineData, index) => {
          const testSignature = lineData.value
    ...
    drawStateVisAxis() {
      ...
      // 文本条显示区域
      const stateVisAxis = d3
        .select('.state-vis-axis')
        .append('svg')
      ...
      // 掩层区域
      const overlay = stateVisAxis.append('g').attr('class', 'overlay');

      // 数据组装临时区域
      const measure = stateVisAxis.append('g').attr('class', 'measure');

      // 文本显示区域
      const textArea = stateVisAxis
        .append('g')
        .attr('class', 'textArea')
        .attr('width', '90%')
        .attr('height', `${24}px`);
      ...
      // 选取文本滚动区域
      const brush = stateVisAxis
        .append('g')
        .attr('class', 'brush')
        .attr('transform', 'translate(35, 0)');

      // 选取文本滚动区域左右侧严格小于
      const bottomBrush = stateVisAxis
        .append('g')
      ...
      const measureText = measure
        ...
        .attr('x', '35')
        .attr('y', '0');
      const jointWords = this.getJointWords(measureText, this.getSentence);
      ...
      wordCellList
        ...
        .attr('width', 30)
        .attr('height', 24);
      wordCellList.select('text').text((d) => d.text);

      // 设置字符缩放，以防有些英文单词过长
      wordCellList.select('text').attr('transform', (d) => {
    ...
    renderBottomBrush(bottomBrush, xScale) {
      const dragging = () => {
        ...
        bottomBrush.selectAll(`.bottomBrushHandle.handle-${subject.handle}`).attr('cx', newBottomX);
      ...
      if (this.getSelectedRange.length) {
        ...
        const bottomBrushHandles = bottomBrush
          .selectAll('.bottomBrushHandle')
          .data(ext, (d) => d.handle);
        bottomBrushHandles.exit().remove();
        bottomBrushHandles
          .enter()
          .append('circle')
          .attr('class', (d) => `bottomBrushDeco bottomBrushHandle handle-${d.handle}`)
        ...
        bottomBrushWiskers
          .enter()
          .append('line')
          .attr('class', (d) => `bottomBrushDeco bottomBrushWisker handle-${d.handle}`)
        ...
        sl.enter()
          .append('path')
          .attr('class', 'bottomBrushDeco signatureLine')
          .merge(sl)
