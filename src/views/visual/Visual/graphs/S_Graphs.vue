/** Copyright 2026 慧知. All Rights Reserved. * * Licensed under the Apache License,
...
<template>
  <div style="position: relative;">
    <div id="test" class="Graph">
      ...
      <iframe id="size-watch" style="width: 100%; height: 0; border: 0;"></iframe>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    ...
    const el = document.querySelector('#size-watch');
  ...
  methods: {
      ...
      const draw = function (data, init) {
        ...
        g.nodes().forEach((v) => {
          ...
          if (node !== undefined) {
            if (node.type == 'VirtualNode') {
              ...
              const style_data = GetStyleByLabel(node.label);
              node.rx = node.ry = 10;
        ...
        svg.selectAll('.node')._groups[0].forEach((nodeKey) => {
          ...
          nodeThis.on('click', () => {
            ...
            const fmType = getFMType();

            if (fmType !== undefined) {
              const tag = d3.select(idTransformerFrontend(`#${nodeKey.id}`)).attr('clicked');
              if (tag === '1') {
                const fmType = getFMType();
                d3.select(idTransformerFrontend(`#${nodeKey.id}`)).attr('clicked', '0');
                // 去除阴影和特征图预览窗口
                ...
                d3.select(idTransformerFrontend(`#${nodeKey.id}`)).attr('clicked', '1');
                const translate_str = d3
                  .select(idTransformerFrontend(`#${nodeKey.id}`))
                  .attr('transform');
                const translate_x =
                ...
                const shadow_width = parseFloat(
                  d3
                    .select(idTransformerFrontend(`#${nodeKey.id}`))
                    .select('.label-container')
                    .attr('width')
                );
                const shadow_height = parseFloat(
                  d3
                    .select(idTransformerFrontend(`#${nodeKey.id}`))
                    .select('.label-container')
                    .attr('height')
                ...
                d3.select(idTransformerFrontend(`#${nodeKey.id}`))
                  ...
                  .attr('opacity', '0.5');
                // 特征图预览窗口添加数据
                const param_tag = `${nodeKey.id}-${fmType}`;
                if (fmType !== '') {
                  const para = {
    ...
    signAvailableNode(type) {
      if (type !== undefined) {
        ...
        this.getFmType[this.userSelectRunFile][type].forEach((nodeId) => {
          const node = d3.select(this.idTransformerFrontend(`#${nodeId}`));
          const rect_width = node.select('rect').attr('width');
          const rect_color = node.select('rect').style('fill');
          const circle = node
            .append('g')
    ...
    edgeLabelAnimation() {
      ...
      edgeGroup.forEach((edge, index) => {
        ...
        const newPath = d3.select(edge).append('path');
        newPath
