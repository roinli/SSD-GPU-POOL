  ...
  <div class="exceptionContainer">
    <el-card style="height: 100%;">
      <div :class="excepContainerHeadShow ? 'excepContainerHead2' : 'excepContainerHead1'">
        <div class="excepHead">
          <span style="font-weight: 600;">{{ oneData[0] }}</span
          >/<span>{{ oneData[1] }}</span>
        </div>
...
export default {
  ...
  watch: {
    ...
    getCurIqrTimes(val) {
      ...
      if (this.myOneData[0] === val[0] && this.myOneData[1] === val[1]) {
        ...
        if (this.myAllStep[this.curStepIndex] !== val[2]) {
          // step发生变化
          let k = 0;
        ...
        if (dq !== 0) {
          // 修改这个子组件中的数据
          this.myBoxPercent[this.curStepIndex][0][0] =
            this.myBoxPercent[this.curStepIndex][0][1] + val[3] * dq;
          this.myBoxPercent[this.curStepIndex][0][4] =
            this.myBoxPercent[this.curStepIndex][0][3] - val[4] * dq;
          // 修改vuex中保存的数据
          this.setAllStepBoxUp({
            index: this.index,
            step: this.curStepIndex,
        ...
        const paramTemp = {
          ...
          step: this.myAllStep[this.curStepIndex],
          down: this.myBoxPercent[this.curStepIndex][0][4],
          up: this.myBoxPercent[this.curStepIndex][0][0],
        };
  ...
  methods: {
    ...
    drawExcepHist() {
      ...
      const histOuterSvg = d3
        .select(`#${this.excepHistId}`)
        .append('svg')
        .attr('width', '100%')
        .attr('height', '100%')
        .attr('preserveAspectRatio', 'xMidYMid meet')
        .attr('viewBox', `0 0 ${histSvgWidth} ${histSvgHeight}`);
      const histSvg = histOuterSvg
        .append('g')
    ...
    CanvasRectMouseOperator() {
      ...
      function onMouseMoveFunc(e) {
        ...
        that.setRectCurInfo([
          that.myOneData[0],
          that.myOneData[1],
          that.myAllStep[that.curStepIndex],
          row,
          col,
    ...
    drawExcepLedgend() {
      ...
      upDownRect
        ...
        .attr(
          'd',
          `m${legendRectWidth} 0L${legendRectWidth + lengthTri} ${-lengthTri}L${legendRectWidth +
            lengthTri} 0 Z`
        )
        ...
        .call(
          d3
            ...
            .on('drag', function _nonName() {
              ...
              d3.select(this).attr(
                'd',
                `m${legendRectWidth} ${0 + upHeight}L${legendRectWidth + lengthTri} ${-lengthTri +
                  upHeight}L${legendRectWidth + lengthTri} ${+upHeight} Z`
              );
      ...
      upDownRect
        ...
        .attr(
          'd',
          `m${legendRectWidth} ${legendRectHeight}L${legendRectWidth +
            lengthTri} ${legendRectHeight + lengthTri}L${legendRectWidth +
            lengthTri} ${legendRectHeight} Z`
        )
        ...
        .call(
          d3
            ...
            .on('drag', function _nonName() {
              d3.select(this).style('cursor', 's-resize');
              if (d3.event.y > legendRectHeight || d3.event.y < upHeight)
                downHeight = legendRectHeight;
              else downHeight = d3.event.y;
              d3.select(this).attr(
                'd',
                `m${legendRectWidth} ${downHeight}L${legendRectWidth + lengthTri} ${lengthTri +
                  downHeight}L${legendRectWidth + lengthTri} ${downHeight} Z`
              );
    ...
    // 画step坐标轴
    drawExcepStepAxis() {
      d3.select(`#${this.excepStepAxisId}`)
        .select('svg')
        .remove();
      ...
      stepXAxis
        ...
        .call(
          brush.move,
          [allStep[that.boxLeftIndex], allStep[that.boxRightIndex]].map(stepXScale)
        ); // 默认选五步
    ...
    drawExcepBox() {
      ...
      const bigBoxSvg = d3
        .select(`#${this.excepBoxId}`)
        .append('svg')
        .attr('width', '100%')
        .attr('height', '100%')
        .attr('preserveAspectRatio', 'xMidYMid meet')
        .attr('viewBox', `0 0 ${bigBoxSvgWidth} ${bigBoxSvgHeight}`)
        .append('g');
      ...
      bigBoxSvg
        .append('g')
        .call(d3.axisBottom().scale(boxXScale))
      ...
      this.boxYScale = d3
        .scaleLinear()
        .domain([valueMin1, valueMax1])
        .range([bigBoxHeight, 0])
        .nice();
      bigBoxSvg
        .append('g')
        .call(
    ...
    drawOneBox(svg, y, index, center, width, top) {
      ...
      const upText = svg
        .append('g')
        .append('text')
        .attr('x', center - width / 3)
        .attr('stroke', 'black')
        .attr('font-size', '10px'); // 上下线的y值
      const downText = svg
        .append('g')
        .append('text')
        .attr('x', center - width / 3)
        ...
        .call(
          d3
            ...
            .on('end', function _nonName() {
              ...
              d3.select(`#${that.excepHistId}`)
                .select('svg')
                .select('.minLine')
                .attr('x', that.histxScale(y.invert(curMinPosition - top)));
            })
        );
      // max
          ...
          d3
            ...
            .on('end', function _nonName() {
              ...
              d3.select(`#${that.excepHistId}`)
                .select('svg')
                .select('.maxLine')
                .attr('x', that.histxScale(y.invert(curMaxPosition - top)));
            })
        );
      // 六个百分比
      svg
        .append('g')
        .selectAll('circle')
        .data(boxPercent[index][1])
        .enter()
        .append('g')
        .append('circle')
        .attr('r', '3')
        .attr('cx', center)
        .attr('cy', function _nonName(d) {
          return top + y(d);
        })
        .attr('fill', 'blue')
        .style('opacity', '0.5');
        ...
        svg
          ...
          .on('mouseover', function _nonName(d, i) {
            ...
            that.setRectCurInfo([
              ...
              curRow,
              curColumn,
              that.oneData[3][4][curRow][curColumn],
            ]);
          });
      }
    },
    ...
    setRectExcepBox() {
      ...
      if (this.curExcepBox.length) {
        ...
        this.setExcepBoxStatistic([
          this.curExcepBox[0].length,
          (
            (this.curExcepBox[0].length * 100.0) /
            (this.myOneData[3][0][0] * this.myOneData[3][0][1])
          ).toFixed(2),
        ]);
      }
    },
