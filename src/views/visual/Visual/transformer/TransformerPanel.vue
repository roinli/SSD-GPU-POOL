...
<template>
  <div class="temp">
    <div v-if="reload" class="transformer-panel">
            ...
            <div class="info-content">
              <div class="info-content-item">
                ...
                <el-select v-model="selectImg" placeholder="请选择">
                  <el-option v-for="(item, i) in imgList" :key="i" :label="item" :value="item">
                  </el-option>
                </el-select>
              </div>
              <div class="info-content-item">
                ...
                <el-select v-model="selectLayer" multiple>
                  <el-checkbox
                    v-model="allLayerChecked"
                    style=" width: 95%; text-align: right;"
                    :indeterminate="
                      layerList.length !== selectLayer.length && selectLayer.length > 0
                    "
                    @change="allLayerCheckedChange"
                    >全选</el-checkbox
                  >
              ...
              <div class="info-content-item">
                ...
                <el-switch
                  v-model="normalize_type"
                  style="display: block; margin: 0 auto;"
                  active-color="#625eb3"
                  inactive-color="#625eb3"
                  active-text="局部"
                  inactive-text="全局"
                >
                </el-switch>
              </div>
              <div v-show="!normalize_type" class="info-content-item">
...
export default {
    ...
    getSentences(val) {
      ...
      if (this.textList.length > 2) {
        for (let i = 0; i < this.textList.length; i++) {
          const text = { id: i, value: this.textList[i] };
  ...
  methods: {
    ...mapTransformerActions(['getSentencesData', 'getTransformerTextData', 'fetchAttentionMap']),
    ...
    getAttentionMap() {
      this.selectLayer.forEach((el) => {
        const arr = el.split('-');
        const layer = String(Number(arr[1]) - 1);
