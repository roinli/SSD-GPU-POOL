...
export default {
  ...
  methods: {
    ...
    renderInit() {
      if (JSON.stringify(this.localCurInfo) === '{}') {
      } else if (parseInt(this.localCurInfo.curDim) === 3) {
        // 是3维显示
    ...
    renderTwoInit(data) {
      ...
      var brush = d3
        ...
        .on('end', () => {
          const s = d3.event.selection; // brushended 会触发两次为什么么
          if (!s) {
    ...
    renderNInit(data) {
      ...
      const gMain = svg
        ...
        .attr('id', 'mainGroup')
        .attr('transform', `translate(${vm.margin.left}, ${vm.margin.top})`);
      gMain.append('g').attr('id', 'background');
      gMain.append('g').attr('id', 'foreground');
    ...
    renderNUpdate(localData) {
      const vm = this;
      const x = d3
        .scalePoint()
        .rangeRound([0, vm.width])
        .padding(1);
      const y = {};
      const dragging = {};
      const data = [];
      let dimensions;
      const line = d3.line();
      const myEPS = 0.0000000001;
      const minValue = {}; // 每个维度对应的最大最小值
      const maxValue = {};
      const str = [
        'a',
        'b',
      ...
      dimensions.forEach(function(d) {
        ...
        if (vals.every(quantP)) {
          maxValue[d] = d3.max(data, function(p) {
            return +p[d];
          });
          minValue[d] = d3.min(data, function(p) {
            return +p[d];
          });
          y[d] = d3
            .scaleLinear()
      ...
      // background
      vm.ParaCoor.backgroundUpdate = gMainBackground.selectAll('path').data(data);
      vm.ParaCoor.backgroundExit = vm.ParaCoor.backgroundUpdate.exit();
      vm.ParaCoor.backgroundEnter = vm.ParaCoor.backgroundUpdate
        .enter()
        .append('path')
        .attr('d', path)
        .attr('fill', 'none')
        .attr('stroke-width', vm.getLineWidth.toString());
      vm.ParaCoor.backgroundUpdate
        .merge(vm.ParaCoor.backgroundEnter)
        .transition()
        .ease(d3.easeLinear)
        .duration(1000)
        .attr('d', path)
      ...
      vm.ParaCoor.foregroundUpdate = gMainFroeground.selectAll('path').data(data);
      vm.ParaCoor.foregroundExit = vm.ParaCoor.foregroundUpdate.exit();
      vm.ParaCoor.foregroundEnter = vm.ParaCoor.foregroundUpdate
        .enter()
        .append('path')
        .attr('d', path)
        .attr('fill', 'none')
      ...
      vm.ParaCoor.foregroundUpdate
        .merge(vm.ParaCoor.foregroundEnter)
        .transition()
        .ease(d3.easeLinear)
        .duration(1000)
        .attr('d', path)
      ...
      vm.ParaCoor.dimensionsEnter = vm.ParaCoor.dimensionsUpdate
        .enter()
        .append('g')
      ...
      vm.ParaCoor.dimensionsUpdate
        .merge(vm.ParaCoor.dimensionsEnter)
        .transition()
        .ease(d3.easeLinear)
        .duration(1000);
      gMain
        ...
        gMain
          ...
          .each(function(d) {
            ...
            data.forEach(function(p, index) {
              for (let in2 = 1; in2 < tmp.length; in2++) {
                // 比较耗时的计算每个间隔的每个分类的个数
            ...
            d3.select(this)
              ...
              .each(function(datum, index) {
                // 一个对象里面存着所有要显示的标签的统计数据 也就是一个小ticks中要显示的所有数据
                ...
                for (let in1 = 0; in1 < tmpKeys.length; in1++) {
                  d3.select(this)
                    ...
                    .attr('x', function(datum) {
                      ...
                      if (in1 === 0) {
                        ...
                        xTmp =
                          ((-1 * xTmp) / localData.echaLabelNumber[tmpKeys[in1]]) * x.step() * 0.2;
                      }
                      return xTmp;
                    }) // x 要动
                    .attr('width', function(datum) {
                      return (
                        (datum[tmpKeys[in1]] / localData.echaLabelNumber[tmpKeys[in1]]) *
                        x.step() *
                        0.2
                      );
                    })
                  ...
                  d3.select(this)
                    ...
                    .attr('d', function(datum) {
                      ...
                      rlt = `m${xTmp} ${y[d](tmp[index + 1]) + oneHight * 0.1} L${xTmp +
                        letWidth} ${y[d](tmp[index + 1]) + oneHight * 0.1} L${xTmp + letWidth} ${y[
                        d
                      ](tmp[index + 1]) +
                        oneHight * 0.1 +
                        oneHight * 0.8} L${xTmp} ${y[d](tmp[index + 1]) +
                        oneHight * 0.1 +
                        oneHight * 0.8} Z`;
                      return rlt;
                    })
      ...
      const legend = d3
        ...
        .attr('transform', function(d, i) {
          return `translate(${vm.width -
            vm.margin.left -
            vm.margin.right -
            80},${-(i + 1) * 25 + vm.height - vm.margin.bottom - vm.margin.top})`;
        });
    ...
    renderTwoUpdate(localData) {
      ...
      this.dataAttr.minX = d3.min(localData.data, function(d) {
        return d[0];
      });
      this.dataAttr.maxX = d3.max(localData.data, function(d) {
        return d[0];
      });
      this.dataAttr.minY = d3.min(localData.data, function(d) {
        return d[1];
      });
      this.dataAttr.maxY = d3.max(localData.data, function(d) {
        return d[1];
      });
      this.height = d3
        .select(this.$refs.draw)
    ...
    renderUpdate(localData) {
      ...
      if (parseInt(this.localCurInfo.curDim) === 3) {
        // 是3维显示
