...
import { deleteFile as deleteFileApi, save as saveApi } from '@/api/preparation/textData';
import { getAudioDetail } from '@/api/preparation/datafile';
import { fileCodeMap, dataTypeMap, annotationBy, annotationMap } from '../../util';
import WorkSpace from '../workspace';
import SideBar from '../../nlp/annotation/sidebar';
...
export default {
  ...
  setup(props, ctx) {
    ...
    const queryFileUtil = (cfg) => {
      ...
      return getAudioDetail({ datasetId: params.datasetId, ...requestParams });
    };
    ...
    const showLabel = computed(
      () => state.datasetInfo.annotateType === annotationMap.AudioClassify.code
    );
    const datasetUrl = `/data/datasets/audio/list/${params.datasetId}`;
    ...
    onMounted(async () => {
      ...
      if (![dataTypeMap.AUDIO].includes(datasetInfo.dataTypeMap)) {
