...
<template>
  <div>
    ...
    <div class="classify-container flex">
      ...
      <div class="file-list-container">
        <div v-loading="crud.loading" class="app-container">
          ...
          <div v-if="lastTabName === 'noAnnotation' && crud.page.total === 0 && !crud.loading">
            <InfoCard :handleClick="openUploadDialog">
              <span slot="desc">点击添加图片</span>
            </InfoCard>
          </div>
          <div v-if="lastTabName === 'haveAnnotation' && crud.page.total === 0 && !crud.loading">
            <InfoCard>
        ...
        <div class="fixed-label-list">
          ...
          <div class="mb-10">
            ...
            <div class="f14">
              ...
              <span v-if="datasetInfo.annotateType && templateTypeName" class="vm">
                ({{ templateTypeName }})
              </span>
            </div>
          </div>
          ...
          <div v-if="rawLabelData.length">
            ...
            <div v-if="lastTabName === 'haveAnnotation'" class="label-switch-tool">
    ...
    <PicInfoModal
      ...
      :handleOk="handlePicModalClose"
    />
  </div>
</template>

<script>
import { without, pick, isNil, unionBy } from 'lodash';
...
import LabelTip from '@/views/dataset/annotate/settingContainer/labelTip';
import PicInfoModal from '@/views/dataset/components/picInfoModal';
import EditLabel from '@/views/dataset/annotate/settingContainer/labelList/edit';
...
export default {
  ...
  computed: {
    formItems() {
      return this.lastTabName === 'noAnnotation' ? this.formItemsUnfinish : this.formItemsFinish;
    },
    // 文件上传前携带尺寸信息
    withDimensionFile() {
    ...
    selectedItems() {
      return this.crud.data.filter((d) => this.selectImgsId.includes(d.id));
    },
    countInfoTxt() {
      const { noAnnotation, haveAnnotation } = this.countInfo;
      return {
        noAnnotation: isNil(noAnnotation) ? `无标注信息` : `无标注信息（${noAnnotation}）`,
        haveAnnotation: isNil(haveAnnotation) ? `有标注信息` : `有标注信息（${haveAnnotation}）`,
      };
    },
    // 预置数据集不支持新建标签
    showAddLabel() {
    ...
    // 标签模板（单标签、多标签）
    templateTypeName() {
      if (this.templateType === 'single-label') return '单标签';
      if (this.templateType === 'multiple-label') return '多标签';
      return null;
    },
    // 展示标签编辑列表
    showEditLabels() {
      return this.lastTabName === 'haveAnnotation' && this.selectImgsId.length > 0;
    },
  },
  created() {
  ...
  methods: {
    ...
    handleEditLabel(field, item) {
      editLabel(item.id, field, this.datasetId).then(this.refreshLabel);
    },
    // 从选中的图片中，将已标记的某标签修改为另一个标签
    handleUpdateLabels(newLabelId, oldLabelId) {
      try {
        // 获取文件上原有的标注信息 替换相应的标注信息
        const annotations = this.selectImgsId.map((id) => {
          const oldFile = this.crud.data.filter((item) => item.id === id);
          const oldAnnotation = JSON.parse(oldFile[0].annotation);
          const newAnnotation = oldAnnotation.map((d) => {
            if (d.category_id === oldLabelId) {
              d.category_id = newLabelId;
            }
            return d;
          });
          return {
            annotation: JSON.stringify(newAnnotation),
            id,
          };
        });
        this.updateImgsLabel(annotations);
      } catch (err) {
        console.error(err);
      }
    },
    ...
    handleRemoveLabels(labelId) {
      try {
        // 获取文件上原有的标注信息 去除要删除的标注信息
        const annotations = this.selectImgsId.map((id) => {
          const oldFile = this.crud.data.filter((item) => item.id === id);
          const oldAnnotation = JSON.parse(oldFile[0].annotation);
          const newAnnotation = oldAnnotation.filter((d) => d.category_id !== labelId);
          return {
            annotation: JSON.stringify(newAnnotation),
            id,
          };
    ...
    chooseLabel(label, event) {
      ...
      if (this.selectImgsId.length > 0) {
        if (this.templateType === 'single-label') {
          const annotations = this.selectImgsId.map((id) => {
            const oldFile = this.crud.data.filter((item) => item.id === id);
            // 如果新增的标签原来已经标注，则该文件无需更新标注信息
            if (!isNil(oldFile[0].labelId) && oldFile[0].labelId.includes(label.id)) {
              return null;
            }
            return {
          ...
          this.updateImgsLabel(annotations);
        } else if (this.templateType === 'multiple-label') {
          const annotations = this.selectImgsId.map((id) => {
            const oldFile = this.crud.data.filter((item) => item.id === id);
            // 如果新增的标签原来已经标注，则该文件无需更新标注信息
            if (!isNil(oldFile[0].labelId) && oldFile[0].labelId.includes(label.id)) {
            ...
            // 获取文件上原有的标注信息 与新添加的标签信息合并
            let oldAnnotation = [];
            try {
              if (!isNil(oldFile[0].annotation)) {
                oldAnnotation = JSON.parse(oldFile[0].annotation);
              }
            } catch (err) {
              console.error(err);
            }
            // 合并后的标签信息
            const unionedAnnotation = unionBy(
              [{ category_id: label.id, score: 1.0 }],
              oldAnnotation,
              'category_id'
            );
            return {
              annotation: JSON.stringify(unionedAnnotation),
              id,
            };
          });
          // 选中的文件中，有的文件原标注中已经有这次要新增的标签了，无需更新该文件的标注，现在不更新的文件在数组中是null，要过滤掉再提交
          const filteredAnnotations = annotations.filter((d) => !isNil(d?.id));
          this.updateImgsLabel(filteredAnnotations);
        }
      } else {
        this.$message({
          message: '没有选中任何图片',
