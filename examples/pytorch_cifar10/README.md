# Examples for opensourceai (Pytorch)
This project provides the examples to run on the [opensourceai](https://github.com/opensource-china/pai). The sample program supports CPU, GPU, multiple GPU, [Horovod](https://github.com/horovod/horovod) modes to train a classification model for cifar10. Through this project, users can quickly and easily run an opensourceai instance, which can be used to learn and understand the opensourceai, or test the performance of the opensourceai.
...
## Usage
To quickly submit a CPU/GPU/Horovod training job to the opensourceai cluster, users can directly submit the corresponding yaml file as mentioned above (in the yaml folder). However, we cannot show all possible resource allocations and networks in the example yaml files. Therefore, users can customize the training jobs accoding to the following two steps.
