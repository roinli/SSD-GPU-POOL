...
from deployment.utility.ssh import opensourceaiSSH
from deployment.utility.sftp_copy import opensourceaiSftpCopy
...
class UtilityCmd():
    ...
    def cluster_ssh(self, args):
        ...
        ssh_handler = opensourceaiSSH(args.command, machine_list, args.filter)
        ssh_handler.run()
    ...
    def cluster_sftp_copy(self, args):
        ...
        if args.dest != None and os.path.isabs(args.dest) is not True:
            ...
            sys.exit(1)
        self.rule_check(args.filter)
        sftp_copy_handler = opensourceaiSftpCopy(args.file_name, args.source, args.dest, machine_list, args.filter)
